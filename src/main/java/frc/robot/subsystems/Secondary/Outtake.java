...
public class Outtake extends SubsystemBase {
    ...
    private SparkFlex wheelSpeedMtr;
    public AbsoluteEncoder wheelSpeedEnc;
    public SparkClosedLoopController wheelSpeedPID;
    private SparkFlexSim wheelSpeedMtrSim;
    private SparkAbsoluteEncoderSim wheelSpeedEncSim;
    private SparkFlexConfig wheelSpeedMtrCfg;
    ...
    public Outtake() {
        wheelSpeedMtr = new SparkFlex(1, MotorType.kBrushless); //TODO change port later
        wheelSpeedMtrCfg = new SparkFlexConfig();

        wheelSpeedPID = wheelSpeedMtr.getClosedLoopController();
        wheelSpeedEnc = wheelSpeedMtr.getAbsoluteEncoder();
        ...
        wheelSpeedMtrCfg.absoluteEncoder
                .positionConversionFactor(360)
                .inverted(false)
                .zeroOffset(0.34722222);
        wheelSpeedMtrCfg.softLimit
                .forwardSoftLimit(200.0)
                .reverseSoftLimit(60.0)
                .forwardSoftLimitEnabled(true)
                .reverseSoftLimitEnabled(true);
        wheelSpeedMtrCfg.closedLoop
                .pid(angkP, angkI, angkD)
                .outputRange(angOutputMin, angOutputMax)
                .feedbackSensor(FeedbackSensor.kAbsoluteEncoder);
        wheelSpeedMtr.configure(wheelSpeedMtrCfg, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);

        // Add motors to the simulation
        if (Robot.isSimulation()) {
            wheelSpeedMtrSim = new SparkFlexSim(wheelSpeedMtr, DCMotor.getNEO(1));
            wheelSpeedEncSim = new SparkAbsoluteEncoderSim(wheelSpeedMtr);
            wheelSpeedMtrSim.setVelocity(190);
            wheelSpeedEncSim.setVelocity(190);
            wheelSpeedMtrSim.setVelocity(0);
            wheelSpeedEncSim.setVelocity(0);
        }
    }
    ...
    public FunctionalCommand setVelocityCmd(double vel) {
        return new FunctionalCommand(
                ...
                () -> (Math.abs(vel - wheelSpeedEnc.getVelocity()) <= 50),
                this);
    }

    public void setVelocity(double velocity) {
        wheelSpeedPID.setReference(velocity, SparkMax.ControlType.kVelocity);
    ...
    public void simulationPeriodic() {
        ...
        wheelSpeedEncSim.setVelocity(wheelSpeedMtrSim.getVelocity());
        wheelSpeedMtrSim.iterate(wheelSpeedEncSim.getVelocity(), wheelSpeedMtrSim.getBusVoltage(), .005);
    }
    ...
    public void periodic() {
        ...
        if (Robot.isSimulation()) {
            SmartDashboard.putNumber("Cool Wheel Position", wheelSpeedEncSim.getVelocity());
        } else {
            SmartDashboard.putNumber("Cool Wheel Position", wheelSpeedEnc.getVelocity());
            SmartDashboard.putNumber("Cool Wheel Speed", wheelSpeedEnc.getVelocity());
